...
# ── 차종 보정 계수 (2026-06-09) ──────────────────────────────────────────────
# VT-Micro 원계수(Ahn 보정 차량)는 현 국산 4등급 승용보다 ~15% 무겁다.
# 정속 80km/h 연비 12.9 → 15.0 km/L(4등급 고속연비 톤)로 맞추도록 출력에 곱한다.
#   k = 12.9 / 15.0 ≈ 0.86. 모든 연료(주행+공회전)에 일관 적용 → 상대 비교 불변,
#   절대 연료비만 현실 톤으로 보정. (재학습 시 reward 스케일 변하므로 평가/시뮬 기준.)
FUEL_CALIB = 0.86
...
def fc_rate(v_ms: float, a_ms2: float) -> float:
    ...
    s_kmh = v_ms * 3.6
    a_kmhs = a_ms2 * 3.6
    return vt_micro_fuel(s_kmh, a_kmhs) * FUEL_CALIB
